/*
 *  Implementation of the general division operation in RNS using floating-point intervals
 *  as described in IEEE Access paper https://ieeexplore.ieee.org/document/9043511
 */
...
namespace cuda {
    ...
    DEVICE_CUDA_FORCEINLINE void rns_div(int *q, int *r, int *x, int *d) {
        ...
        while (cuda::er_ucmp(&reval.low, &deval.upp) >= 0) {
            ...
            cuda::rns_set(pq, cuda::RNS_POW2[flq.exp]); //Fetch the partial quotient, pq, from the LUT
            cuda::rns_add(q, q, pq); //q = q + pq
            cuda::rns_mul(pq, pq, d); //pq = d * pq
            cuda::rns_sub(r, r, pq); // r = r - pq
            cuda::rns_eval_compute(&reval.low, &reval.upp, r);
        }
        //Final adjustment
     ...
     * It eliminates multiple recalculations of the interval evaluation
     */
    DEVICE_CUDA_FORCEINLINE void rns_div_fast(int *q, int *r, int *x, int *d) {
        int pq[RNS_MODULI_SIZE]; //Partial quotient
        er_float_t flq; //The result of floating-point division
        interval_t deval; //Interval evaluation of d
        interval_t reval; //Interval evaluation of r
        ...
        while (cuda::er_ucmp(&reval.low, &deval.upp) >= 0) {
            ...
            while (flq.exp >= 0 && flq.frac > 0){
                cuda::rns_add(pq,pq,RNS_POW2[flq.exp]); //Fetch the partial quotient from the LUT and add it to pq
